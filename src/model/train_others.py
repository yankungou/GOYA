...
from net import make_GOYA, make_model_single_layer, make_model_rm_clf

from pytorch_metric_learning import losses
from triplet_loss import batch_all_cs_triplet_loss, get_soft_cs_triplet_mask
...
def resume_epoch_model_ddp(args, model, optimizer):
    ...
    if args.resume:
        if os.path.isdir(args.resume) or os.path.isfile(args.resume):
            ...
            print('best_content_loss', best_content_loss)
            print('best_style_loss', best_style_loss)
            print('best_content_batch_idx', best_content_batch_idx)
            print('best_style_batch_idx', best_style_batch_idx)
...
def train_model_epoch(args, train_loader, val_loader, model, model_without_ddp, optimizer, epoch, device, best_loss, best_content_loss, best_style_loss, main_loss, cls_loss, best_batch_idx, best_content_batch_idx, best_style_batch_idx, loss_scaler, distance_matrix):
    ...
    content_losses = AverageMeter('Content loss', ':.4e')
    style_losses = AverageMeter('Style loss', ':.4e')

    if args.model in ['model_ntxent', 'model_triplet']:
        style_clf_losses = AverageMeter('Style classification loss', ':.4e')
        progress = ProgressMeter(len(train_loader), 
                                [losses, content_losses, style_losses, style_clf_losses], 
    ...
    with tqdm(postfix=dict, mininterval=0.3) as pbar:
        for batch_idx, (feature_ori, prompt) in enumerate(train_loader):
            ...
            if args.model in ['model_ntxent', 'model_triplet']:
                ...
                if args.model == 'model_ntxent':
                    content_indices_tuple, style_indices_tuple = get_cs_sep_indices_tuples(content_target, style_target, distance_matrix)
                    content_batch_loss = main_loss(out_feature_c, indices_tuple=content_indices_tuple)
                    style_batch_loss = main_loss(out_feature_s, style_target)
    
                elif args.model == 'model_triplet':
                    valid_content_mask, valid_style_mask = get_soft_cs_triplet_mask(content_target, style_target, distance_matrix)
                        
                    content_batch_loss = main_loss(valid_content_mask, out_feature_c, args.margin) 
                    style_batch_loss = main_loss(valid_style_mask, out_feature_s, args.margin)
                ...
                content_losses.update(content_batch_loss.item(), feature_ori.size(0))
                style_losses.update(style_batch_loss.item(), feature_ori.size(0))
                ...
                out_feature_c, out_feature_s, _, _ = model(feature_ori)
                
                if args.model == 'model_contrastive_rm_clf':
                    content_indices_tuple, style_indices_tuple = get_cs_sep_indices_tuples(content_target, style_target, distance_matrix)
                    ...
                    pos_content_loss = main_loss(out_feature_c[content_indices_tuple[0]], out_feature_c[content_indices_tuple[1]], content_pos_target)
                    neg_content_loss = main_loss(out_feature_c[content_indices_tuple[2]], out_feature_c[content_indices_tuple[3]], content_neg_target)
                    content_batch_loss = pos_content_loss + neg_content_loss
                    
                    pos_style_loss = main_loss(out_feature_s[style_indices_tuple[0]], out_feature_s[style_indices_tuple[1]], style_pos_target)
                    neg_style_loss = main_loss(out_feature_s[style_indices_tuple[2]], out_feature_s[style_indices_tuple[3]], style_neg_target)
                    style_batch_loss = pos_style_loss + neg_style_loss
                    
                elif args.model == 'model_ntxent_rm_clf':
                    content_indices_tuple, style_indices_tuple = get_cs_sep_indices_tuples(content_target, style_target, distance_matrix)
                    content_batch_loss = main_loss(out_feature_c, indices_tuple=content_indices_tuple)
                    style_batch_loss = main_loss(out_feature_s, style_target)
                    
                elif args.model == 'model_triplet_rm_clf':
                    valid_content_mask, valid_style_mask = get_soft_cs_triplet_mask(content_target, style_target, distance_matrix)
                
                    content_batch_loss = main_loss(valid_content_mask, out_feature_c, args.margin) 
                    style_batch_loss = main_loss(valid_style_mask, out_feature_s, args.margin)

                elif args.model in ['model_single_256', 'model_single_512', 'model_single_1024', 'model_single_2048']:
                    content_indices_tuple, style_indices_tuple = get_cs_sep_indices_tuples(content_target, style_target, distance_matrix)
                    content_pos_target = torch.ones(content_indices_tuple[0].size(0), device=device)
                    content_neg_target = - torch.ones(content_indices_tuple[2].size(0), device=device)
                ...
                loss = content_batch_loss + style_batch_loss
                ...
                if misc.is_main_process():
                    if curr_iter % batch_val_slot == 0:
                        val_loss, val_content_batch_loss, val_style_batch_loss = val_model(args, val_loader, model, epoch, device, main_loss, cls_loss, distance_matrix)
                        ...
                        print('** Validation: %f (best loss) - %f (current loss)' % (best_loss, val_loss))
        ...
        if misc.is_main_process():
            if args.model in ['model_ntxent', 'model_triplet']:
                epoch_log_stats = {**{'epoch': epoch,
                                    ...
                                    'train epoch content loss': '%0.4f' % content_losses.avg, 
                                    'train epoch style loss': '%0.4f' % style_losses.avg, 
...
def val_model(args, val_loader, model, epoch, device, main_loss, cls_loss, distance_matrix):
    ...
    content_losses = AverageMeter('Content val loss', ':.4e')
    style_losses = AverageMeter('Style val loss', ':.4e')
    
    if args.model in ['model_contrastive_rm_clf', 'model_ntxent_rm_clf', 'model_triplet_rm_clf']:
        val_progress = ProgressMeter(len(val_loader), 
                                [val_losses, content_losses, style_losses], 
                                prefix="Epoch: [{}]".format(epoch))
    else:
        style_clf_losses = AverageMeter('Style classification loss', ':.4e')
        val_progress = ProgressMeter(len(val_loader), 
                                [val_losses, content_losses, style_losses, style_clf_losses], 
                                prefix="Epoch: [{}]".format(epoch))
    ...
    with tqdm(postfix=dict, mininterval=0.3) as pbar:
        ...
        if args.model in ['model_ntxent', 'model_triplet']:
            epoch_log_stats_val = {**{'epoch': epoch,
                        ...
                        'val epoch content_contra_loss': '%0.4f' % content_losses.avg,
                        'val epoch style_contra_loss': '%0.4f' % style_losses.avg,
                        'val epoch style_clf_loss': '%0.4f' % style_clf_losses.avg,
                        }}
            pbar.set_postfix(epoch_log_stats_val)
            val_progress.display_summary()
            return val_losses.avg, content_losses.avg, style_losses.avg, style_clf_losses.avg
            ...
            pbar.set_postfix(epoch_log_stats_val)
            val_progress.display_summary()
            return val_losses.avg, content_losses.avg, style_losses.avg
...
def train(args):
    ...
    elif args.model in ['model_ntxent', 'model_triplet']:
        model = make_GOYA()
    ...
    if args.model in ['model_contrastive_rm_clf', 'model_single_256', 'model_single_512', 'model_single_1024', 'model_single_2048']:
        main_loss = torch.nn.CosineEmbeddingLoss(margin=0.5)
    
    elif args.model in ['model_ntxent', 'model_ntxent_rm_clf']:
        main_loss = losses.NTXentLoss(temperature=0.07)

    elif args.model in ['model_triplet', 'model_triplet_rm_clf']:
        main_loss = batch_all_cs_triplet_loss
